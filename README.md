...
`    behavior: check-daemon.js` — скрипт, который будет запущен как реакция на событие демонов (Внутри `handleEvent` - void метод, который должен вызываться из скрипта чтобы реагировать на событие терминала. Где параметры: `handleEvent(integrationEvent)``, где `integrationEvent` - это js объект, содержащий информацию о событии.)
...
У товара в магазине для одного приложения может быть одно экстра-поле внутри него любой валидный джейсон (например поле с массивом объектов которые могут вам понадобиться на терминале).
...
В пример запроса 1. Изменение схемы продуктов для магазина описаны 2 поля, где 1 поле - текстовое поле, где title = "Произвольный заголовок поля ввода текста" и 2 поле - выбор из списка, где title = "Произвольный заголовок поля выбора из списка"
